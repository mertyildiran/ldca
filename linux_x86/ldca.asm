...
mutate:         mov     ebx, program            ; move cursor to the beginning of the program
                add     ebx, 3                  ; go 3 instructions forward
                ; rdtsc                         ; generate random bytes using CPU's clock (Read Time-Stamp Counter)
                inc     byte [ebx] ; , eax      ; replace the byte with the randomly generated byte
                ret

evolve:         randomNumber    0, 1            ; Generate random number between 0 - 1
                cmp     eax, 1                  ; if generated random number is 1 mutate
                call    mutate                  ; jump to mutate if eax is zero
                ret
