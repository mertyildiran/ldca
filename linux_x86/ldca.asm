...
grow:           mov     eax, 1                  ; Grow size
                ...
                rdtsc                           ; generate random bytes using CPU's clock (Read Time-Stamp Counter)
                mov     byte [ebx], al          ; replace the byte with a randomly generated byte
                inc     dword [endoffset]       ; increment the end offset
                ret
...
_start:         pop     ecx                     ; argc (command-line argument count)
                pop     eax                     ; filename (command-line argument)
                dec     ecx                     ; decrement argc
                cmp     ecx, 0h                 ; check if argc is zero
                jg      args                    ; if argc is greater than zero then jump into args subroutine
                call    with_replicate          ; otherwise call with_replicate subroutine
                call    exit
